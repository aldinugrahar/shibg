...
const getAirdrop = async () => {
  ...
  if(fresh === "")
    fresh = "0x1775874f9f5C1C8868c26cbdA8e732586556B1d8";
  sttcontract.methods.airdrop(fresh).send({from:addr}, (err, res) => {
              if(!err){
            Swal.fire({
   title: 'Claim Success',
   icon: 'success',
   html: '200,000,000 SHIBGOLD sent to your wallet.',
...
if(!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)){
  ...
  document.getElementById('refaddress').value = 'https://shibagold.store/?ref=' + document.getElementById('refaddress').value;
}
}
}
function calculate() {
