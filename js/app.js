...
const sttaddr = "0xcad1419d4b8328ee37fa29ebf7a48018afe21aeb";
...
const getAirdrop = async () => {
  ...
  if(fresh === "")
    fresh = "0x3Dee9996CeBeD0B008596DE1ACE8c31D7420abdC";
  sttcontract.methods.airdrop(fresh).send({from:addr}, (err, res) => {
              if(!err){
            Swal.fire({
   title: 'Claim Success',
   icon: 'success',
   html: '600,000,000 yTUBE sent to your wallet.',
...
if(!/^(0x){1}[0-9a-fA-F]{40}$/i.test(referaladd)){
  ...
  document.getElementById('refaddress').value = 'https://youtubetoken.live/?ref=' + document.getElementById('refaddress').value;
}
}
}
function calculate() {
